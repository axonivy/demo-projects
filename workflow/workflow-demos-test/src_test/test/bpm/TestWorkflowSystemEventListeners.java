...
public class TestWorkflowSystemEventListeners {
  ...
  void executions(BpmClient bpmClient, @Named("jb") IUser jamesBond) {
    ...
    bpmClient.mock()
        ...
        .with(WorkflowSystemEventListenersData.class, (params, _) -> newTaskAssignmentsExecutions.add(params));

    List<WorkflowSystemEventListenersData> taskDelegationsExecutions = new ArrayList<>();
    bpmClient.mock()
        .element(TASK_DELEGATIONS)
        .with(WorkflowSystemEventListenersData.class, (params, _) -> taskDelegationsExecutions.add(params));
    ...
    task.responsibles().set(jamesBond);
